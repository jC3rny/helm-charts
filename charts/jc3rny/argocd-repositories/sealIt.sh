...
# . seal-it.sh -v -c ".repositories" -f ./values-test.yaml -m name -E "date +'repo-%y%m%d-%H%M%S-%3N'" --verbose
...
if [ "$VERBOSE" == "true" ]; then
    ...
    echo "  SEAL_SCRIPT_PATH:   ${SEAL_SCRIPT_PATH}"
...
function seal_value(){
    ...
    elif [ ! -z "${SEAL_SCRIPT_PATH}" ]; then
        printf $(eval "${SEAL_SCRIPT_PATH}" "${1}" "${NAMESPACE}")
